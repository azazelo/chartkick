...
module Chartkick
  module Helper
    ...
    def chartkick_chart(klass, data_source, options, &block)
      ...
      html = (options.delete(:html) || %[<div id="#{element_id}" style="height: #{height}; text-align: center; color: #999; line-height: #{height}; font-size: 14px; font-family: 'Lucida Grande', 'Lucida Sans Unicode', Verdana, Arial, Helvetica, sans-serif;">Loading...</div>]) % {:id => ERB::Util.html_escape(element_id), :height => ERB::Util.html_escape(height)}
